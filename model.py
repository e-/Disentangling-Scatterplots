...
class LargeConvVAE(nn.Module):
    def __init__(self, nb_latents):
        ...
        self.conv1 = nn.Conv2d(1, 32, kernel_size=6,
                               stride=2, padding=2)  # 32 32 32
        self.conv2 = nn.Conv2d(32, 64, kernel_size=6,
                               stride=2, padding=2)  # 64 16 16
        self.conv3 = nn.Conv2d(64, 128, kernel_size=6,
                               stride=2, padding=2)  # 128 8 8
        self.conv4 = nn.Conv2d(128, 256, kernel_size=6,
                               stride=2, padding=2)  # 256 4 4
        ...
        self.fc2 = nn.Linear(nb_latents, 256)
        self.fc3 = nn.Linear(256, 256 * 4 * 4)
        self.deconv1 = nn.ConvTranspose2d(
            256, 128, kernel_size=6, stride=2, padding=2)
        self.deconv2 = nn.ConvTranspose2d(
            128, 64, kernel_size=6, stride=2, padding=2)
        self.deconv3 = nn.ConvTranspose2d(
            64, 32, kernel_size=6, stride=2, padding=2)
        self.deconv4 = nn.ConvTranspose2d(
            32, 1, kernel_size=6, stride=2, padding=2)
    ...
    def encode(self, x):
        ...
        x = self.relu(self.conv3(x))
        x = self.relu(self.conv4(x))
        x = self.relu(self.fc1(x.view(-1, 256*4*4)))
        return self.fc_mean(x), self.fc_std(x)
    ...
    def decode(self, z):
        x = self.relu(self.fc2(z))
        x = self.relu(self.fc3(x))
        x = self.relu(self.deconv1(x.view(-1, 256, 4, 4)))
        x = self.relu(self.deconv2(x))
        x = self.relu(self.deconv3(x))
        return self.sigmoid(self.deconv4(x))
